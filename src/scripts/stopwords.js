export const stopWordsArray = [
    'является', 'являются', 'являлся', 'являлась', 'являлось', 'служит для', 'служат для', 'имеется', 'имеются',
    'обладает', 'обладают', 'считается', 'считаются', 'один из', 'одним из', 'одна из', 'одни из', 'одно из',
    ...
    'вчистую', 'по-любому', 'невероятно', 'неизбежно', 'несомненно', 'близнецы-братья', 'особо', 'предельно',
    'радикально', 'решительно', 'совершенно',
    'сугубо', 'сущий', 'сущая', 'сущее', 'сущие', 'фактически', 'целиком', '99% случаев', 'без малого', 'в некоторых случаях',
    'в районе',     'в среднем', 'в целом', 'всех мастей', 'всяческий',
    ...
    'вырастает в размерах', 'рос в размерах',
    ...
    'вырастает в объеме', 'рос в объеме','увеличился в объеме', 'увеличилась в объеме', 'увеличивается в объеме',
    'увеличилось в объеме', 'увеличенный в объеме', 'дело рук человеческих', 'собираться вместе', 'собираются вместе', 'собирались вместе', 'собрались вместе',
    ...
    'достойный лучшего применения', 'достойные лучшего применения', 'из года в год', 'от раза к разу', 'каждый из нас', 'каждого из нас', 'каждому из нас', 'каждым из нас',
    'имеет место', 'комплекс мер', 'комплекса мер', 'самом деле нет', 'наглядно отражать', 'нагладно отражает', 'наглядно отражают',
    'насчитывать', 'насчитывает', 'насчитывают', 'насчитывается', 'не секрет', 'что ни на есть', 'существенный вклад',
    ...
    'ну', 'работать с мотивацией', 'работаю с мотивацией', 'работаем с мотивацией', 'работаю с мотивацией', 'горячая пора', 'горят глаза',
    'докопаться до истины', 'затронуть вопрос', 'затронут вопрос', 'затронуты вопросы', 'затрагивают вопрос', 'затрагивают вопросы',
    'за каламбур', 'издревле', 'разработнический', 'разработническая', 'в данном тексте', 'данный текст', 'по важнейшим направлениям',
    'бывает так что', 'останавливаться на достигнутом', 'переживать рост', 'переживает рост', 'переживала рост', 'переживали рост',
    'переживают рост', 'пережил рост', 'пережили рост', 'переступить черту', 'переступил черту', 'переступила черту', 'переступило черту',
    'переступает черту', 'переступили черту', 'переступать черту', 'переступят черту', 'перейти в плоскость', 'переход в плоскость', 'перешел в плоскость',
    'перешёл в плоскость', 'перешла в плоскость', 'переходит в плоскость', 'переходят в плоскость', 'перешло в плоскость', 'пески времени',
    'по достоинству оценить', 'оценить по достоинству', 'по достоинству оценен', 'по достоинству оценён', 'по достоинству оценено',
    'по достоинству оценена', 'по достоинству оценены', 'оценён по достоинству', 'оценен по достоинству', 'оценена по достоинству',
    'оценено по достоинству', 'закоулки памяти', 'закоулков памяти', 'закоулкам памяти', 'закоулками памяти', 'закоулках памяти',
    'по мановению волшебной палочки', 'по праву считается', 'по праву считаться', 'по праву считался', 'по праву считалась', 'по праву считалось',
    'по праву считались', 'по словам экспертов', 'повестка дня', 'в качестве', 'в рамках этой темы', 'в рамках этого курса',
    'подвергнуть жесткой критике', 'подвергнуть жёсткой критике', 'подвергать жесткой критике', 'подвергать жёсткой критике',
    'подверг жесткой критике', 'подверг жёсткой критике', 'подвергся жесткой критике', 'подвергся жёсткой критике',
    'подверглись жесткой критике', 'подверглись жёсткой критике', 'подвергнут жесткой критике', 'подвергнут жёсткой критике',
    'подвергнуты жесткой критике', 'подвергнуты жёсткой критике', 'покажет время', 'время покажет', 'последние веяния', 'последних веяний',
    'последним веяниям', 'последними веяниями', 'последних веяниях', 'прямо противоположное', 'прямо противоположного', 'прямо противоположному',
    'прямо противоположном', 'раз за разом', 'расстановка сил', 'расстановки сил', 'расстановке сил', 'расстановкой сил', 'расстановке сил',
    'резонно', 'речи быть не может', 'с пеной у рта', 'счёты с жизнью', 'счеты с жизнью', 'светлая грусть', 'светлой грусти',
    'светлую грусть', 'светлой грустью', "еще пока", "ещё пока", "все знают что", "всем известно что", "все слышали о", "код выше", "код ниже", 
    "кода выше", "кода ниже", "коду выше", "коду ниже", "кодом выше", "кодом ниже"
    ];
